...
namespace CMU462 {
	namespace Collada {
		...
		int ColladaParser::load(const char* filename, SceneInfo* sceneInfo, double scale_factor) {
			...
			if (XMLElement* e_scene = get_element(root, "scene/instance_visual_scene")) {
				...
				while (e_node) {
					parse_node(e_node, scale_factor);
		...
		void ColladaParser::parse_node(XMLElement* xml, double scale_factor) {
		  ...
		  // transformation list
			...
			while (e_child) {
				parse_node(e_child, scale_factor);
			...
			if (e_camera) {
				...
				if (get_element(e_geometry, "mesh")) {
					...
					parse_polymesh(e_geometry, *polymesh, scale_factor);
		...
		void ColladaParser::parse_polymesh(XMLElement* xml, PolymeshInfo& polymesh, double scale_factor) {
