...
namespace CMU462 {
	...
	public:
		/**
		 * Gets bar by ID, starting at 0, if bar exist, a Pointer is returned 
		 * and status is 1, else if no such bar exists nullptr is returned and 
		 * status is -1.
		 */
		TwBar *get_bar(unsigned int ID, int *status);

		TwBar *create_new_bar(std::string name, int *status);
	...
	private:
		...
		// some more, maybe needed for AntTweakBar
		static void on_char_callback(GLFWwindow* window, unsigned int codepoint);
