...
Color Color::fromHex( const char* s ) {
  ...
  Color c;
  c.r = static_cast<float>((rgb & 0xFF0000) >> 16) / 255.0f;
  c.g = static_cast<float>((rgb & 0x00FF00) >> 8) / 255.0f;
  c.b = static_cast<float>((rgb & 0x0000FF) >> 0) / 255.0f;
  c.a = 1.0f; // set alpha to 1 (opaque) by default
